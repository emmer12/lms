<template>
    <div v-if="alertStore.hasAlert()">
        <div v-if="alertStore" :class="wrapperClass" role="alert">
            <template v-if="alertStore.isError()">
                <template v-if="alertStore.hasMultiple()">
                    <strong class="font-bold mr-2">{{
                        trans("global.phrases.argh")
                    }}</strong>
                    <span class="block sm:inline">{{
                        trans("global.phrases.fix_errors")
                    }}</span>
                ...
                <template v-else>
                    ...
                    <span class="block sm:inline">{{
                        Array.isArray(alertStore.messages[0])
                            ? alertStore.messages[0][0]
                            : alertStore.messages[0]
                    }}</span>
                    <span
                        class="absolute top-0 bottom-0 right-0 px-4 py-3 cursor-pointer"
                        @click="removeMessage"
            ...
            <template v-else-if="alertStore.isInfo()">
                <strong class="font-bold mr-2">{{ trans("Opp!") }}</strong>
                <span class="block sm:inline">{{
                    Array.isArray(alertStore.messages[0])
                        ? alertStore.messages[0][0]
                        : alertStore.messages[0]
    ...
    setup(props, { emit }) {
        ...
        const wrapperClass = computed(() => {
            if (alertStore.isSuccess()) {
                ...
                return "border px-4 py-4 rounded relative text-sm text-orange-700 bg-orange-100 border-orange-400";
            } else {
        ...
        const errorKeys = computed(() => {
            if (
                !alertStore.messages ||
                getType(alertStore.messages) === "string"
            ) {
                return null;
            }
